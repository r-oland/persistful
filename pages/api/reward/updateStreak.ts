...
const updateActiveReward = async (
  session: Session,
  newStreak: number,
  oldStreak: number
) => {
  ...
  if (!user?.activeReward) return false;
  ...
  if (completedCycles === reward.totalCycles) return true;
  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      if (oldStreak !== newStreak) {
        const didCompleteReward = await updateActiveReward(
          session,
          newStreak,
          oldStreak
        );

        completedReward = didCompleteReward;
      }
