...
export default async function handler(
  ...
  try {
    ...
    // Up until this point, we have checked if the streak was broken yesterday to prevent the fetching of all days.
    // Why this might be a bad practice: the logic that validates the streak is different from the logic below here.
    ...
    // Generate a list of all potential second chance dates
    ...
    // Feels weird to have separate checks for yesterday and day before yesterday. Why not check with the rest?
    // Probably done for optimization, but again the same problem as above.
    ...
    // Can't we combine the finding of the index with the resetting of the streak. This would make the code more readable.

    // Breaking behavior: Does not keep in mind days that were completely skipped
    ...
    // All of this logic seems ugly, is there not a cleaner way to do thi?
